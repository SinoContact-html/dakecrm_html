...
AlexEdmEdit.prototype = {
	...
	templatePop_product_Direct:[
        "<div class='table_workspace_infoBox listBox'>",
		"<div class='addProductBox addProductBox2'>",
        "<div class='producr_list_self'><input name='addProduct' style='width:540px;' class='text' type='text' value='请输入宝贝ID/链接'>&nbsp;<input type='submit' class='submit' value='添加宝贝'></div>",
        "</div>",
		"<div class='loadingBox'><div class='loadingImg'></div></div>",
        "<ul id='{id}' class='producr_list_Direct'>",
        "</ul>",
        "<div class='pagefen'>",
        "</div>",
        "</div>",
    ].join(""),
	templatePop_product_Direct_list:[
        "<li>",
		"<img src='{pic_url}' width='149' height='149' border='0' />",
        "</li>",
    ].join(""),
    templatePop_pageFen:[
    ...
    template_productTd:[
        ...
        "</p>",
        "<p style='width:{width}px; font-size:12px; line-height:20px; height:60px; overflow: hidden; text-align:center; padding:10px 0; margin:0 {marginLR}px {marginBottom}px {marginLR}px; background:#fff;'>",
        "{title}<br /><span style='font-weight:bold; color:#F60;'>劲爆促销价：￥{price}元</span>",
        "</p>"
    ].join(""),
    template_pdEmpty:[
    ...
    init  :function(obj){
		...
		this._pdAllListUrl = obj.pdAllListUrl;//产品列表接口(所有产品，用于点选产品)
    ...
    initialize : function(){//初始化产品表格
        var self = this;
		var initializeTrue = false;
        var productBlock = new Array();
        //for (var i in this._block){//获得产品区块个数
		for (var i=0; i<=this._block.length-1; i++){
    ...
    initTableBg : function(){
        ...
        //for(var i in productBlock){//IE8不支持这种JS循环语法
		for(var i=0;i<=productBlock.length-1;i++){
            $('#'+this._id+' .'+this._block[productBlock[i]].name).css('background-color',this._block[productBlock[i]].setup.background)
        }
    },
    getBlockEvent : function(){//初始化区块
        //for (var i in this._block){
		for (var i=0; i<=this._block.length-1; i++){
            if (this._block[i].type != 'product_Direct')this.formatEditMenuBox(i);//初始化编辑按钮
            this.blockEvent_hover(i);//区块事件
            if (this._block[i].type != 'product_Direct')this.editMenu_click(i,this._block[i].type);//编辑按钮点击事件
            if(this._block[i].power == 0 || this._block[i].power == 2){
    ...
    blockEvent_hover : function(n){//区块事件
        ...
        $('#'+this._id+' .'+this._block[n].name+' div.tdbox').hover(
            function(){
				...
				if (self._block[n].type == 'product_Direct'){
					self.typeFun_product_Direct(n);
				}
            },
            function(){
		...
		if (self._block[n].type == 'product_Direct'){
			this.typeFun_product_Direct_click(n);
		}
    },
    ...
    typeFun_img : function(n){//图片类型功能
		...
		//给弹出窗口内的input写入原始图片地址和超链接
		$('#'+self._popId+' .popbox_workspace .popbox_frombox dl.uploadImglink_dl dd input[name=image_url]').val($('#'+self._id+' .'+self._block[n].name+' div.tdbox .tableBox img').attr('src'));
		var oldImageLink = null;
		if($('#'+self._id+' .'+self._block[n].name+' div.tdbox .tableBox > a').length > 0){//判断图片是否带链接
			var thisImgLink = $('#'+self._id+' .'+self._block[n].name+' div.tdbox .tableBox a').attr('href');
			$('#'+self._popId+' .popbox_workspace .popbox_frombox dl dd input[name=imagelink]').val(thisImgLink);
			oldImageLink = thisImgLink;
        }else{
			oldImageLink = '';
		}
        ...
        $('#imgSubmit').click(function(){
            if (self._tempVal != '' || oldImageLink != $('input[name=imagelink]').val()){//判断表单内容是否有更改
				if (self._tempVal == ''){
					self._tempVal = $('#'+self._id+' .'+self._block[n].name+' div.tdbox .tableBox').html();
				}
                if($('input[name=imagelink]').val()){//判断是否填写了链接参数
    ...
    typeFun_product : function(n){//产品类型功能
		...
		if(this._block[n].power == 2){
			$(popbox).children('.tab_box').hide();
			$(popbox).children('.listBox').hide();
			$(popbox).children('.setupBox').show();
		}
        this.popPdTabClick();//添加pop框tab点击事件(切换tab)
        $("#tbBackground").bigColorpicker("tbBackground","L");
        if(this._block[n].power == 1 || this._block[n].power == 2){//判断产品编辑状态是否打开
            if("undefined" == typeof this._productArray[n]){//产品列表内容为空时，不执行产品列表初始化
                ...
                }else{//当产品区块为默认产品区块时（2014.11.3修改，增加产品添加功能，原先是没有的）
					this.addPopPdAddBox();//添加pop框自定义产品添加框
                    this.addPopPdAddBoxEvent(n);//添加pop框自定义产品添加框点击事件
				}
        ...
        }else if(this._block[n].power == 0){
            objDemo = {};
            $(popbox).html(this.substitute(this.template_productList_demo,objDemo));//不可编辑时显示提示内容
        }
        $('#productSubmit').click(function(){
            if(self._block[n].power == 1 || self._block[n].power == 2){//判断是否运行进行产品编辑(来自模板列表的不允许编辑)
	...
	typeFun_product_Direct : function(n){//产品点选类型功能
		var self = this;
		var popbox = $('#'+this._popId+' .popbox_workspace');
        obj_product = {id:'allPdList_'+n}
        $(popbox).html(this.substitute(this.templatePop_product_Direct,obj_product));
		$('#'+self._popId+' .popbox_head span.title').html('添加宝贝');
	},
	typeFun_product_Direct_click : function(n){//产品点选类型功能点击事件
		var self = this;
		var id = 'allPdList_'+n;
		var box = $('#'+self._id+' .'+self._block[n].name+' div div table.productTable tbody > tr > td');
		for(i=0; i<=box.length-1; i++){
			if ("undefined" == typeof $(box[i]).attr('class') || "" == $(box[i]).attr('class')){
				$(box[i]).addClass('tb_'+i);
			}
		}
		$(box).click(function(){
			$(this).children('p').children('a').click(function(event){//拦截a标签超链接
				event.preventDefault();
			})
			var td_i = $(this).attr('class');
			td_i = td_i.replace(/tb_/, '');
			self.typeFun_product_Direct_pdAdd_ajax(1,n,id,td_i,true);//加载弹窗内的产品列表,参数说明：1:当前页，默认第一页,n:模块号，td_i:当前点击的td序号
		})
	},
	typeFun_product_Direct_pdAdd_ajax : function(p,n,boxID,td_i,first){//加载弹窗内的产品列表,p:当前页,n:模块号,td_i:当前点击的td序号,first:是否分页按钮点击加载
		var self = this;
		var popbox = $('#'+this._popId+' .popbox_workspace .table_workspace_infoBox');
		$('#'+boxID).html('');
		$('#'+boxID).hide();
		$('#'+boxID).prev().show();
        var params = {pagNum:p};
        $.post(self._pdAllListUrl,params,function(data){
            if(data.enterFlag){
                if (data.pdlist == ''){
					alert('您的店铺中没有宝贝！');
                }
                else {
					//for(var i in data.pdlist){
					for(var i=0; i<=data.pdlist.length-1;i++){
						obj = {};
						obj.pic_url = data.pdlist[i].pic_url;
            			$('#'+boxID).append(self.substitute(self.templatePop_product_Direct_list,obj));//不可编辑时显示提示内容
					}
					self.typeFun_product_Direct_pdAdd_getTable(boxID,data.pdlist,n,td_i);//加载弹窗内产品图点击事件
					$('#'+boxID).show();
					$('#'+boxID).prev().hide();
					if(first){
						self.popPdPagefenGet(data.allpage);//加载分页按钮
						self.popPdDirectClick(1,n,boxID,td_i);//加载产品分页功能
						self.popPdDirectSearchAdd(n,td_i);//搜索添加产品
						self.popOpen(n);//打开弹窗
					}
                }
            }else{
	...
	typeFun_product_Direct_pdAdd_getTable : function(boxID,pdlist,n,td_i){//弹窗内产品图点击事件
		var self = this;
		$('#'+boxID).children('li').click(function(){
			self.getPdDirectAddToTd(n,td_i,pdlist[$(this).index()].title,pdlist[$(this).index()].pic_url,pdlist[$(this).index()].detail_url,pdlist[$(this).index()].price);
		})
	},
            ...
            if($(popbox).children('input[name=addProduct]').val() == ''){
                ...
                if(id == 'error'){//获取并验证宝贝ID为数字类型
                    ...
                    if(self._productArray[n].length >= self._block[n].setup.pdNum){
                        ...
                        if(repeatFlag == false){
                            self.ajaxAddProduct(n,id,'old');//Ajax添加宝贝
                        }
                    }
                }
            }
	...
	popPdDirectSearchAdd : function(n,td_i){
		var self = this;
        var popbox = $('#'+this._popId+' .popbox_workspace .listBox .addProductBox .producr_list_self');
        $(popbox).children('input[name=addProduct]').click(function(){
            if($(this).val() == '请输入宝贝ID/链接'){
        ...
        $(popbox).children('input.submit').click(function(){
            if($(popbox).children('input[name=addProduct]').val() == ''){
                ...
                if(id == 'error'){//获取并验证宝贝ID为数字类型
					...
					self.ajaxAddProduct(n,id,td_i)//区块、产品ID、写入的td编号
                }
            }
        });
	},
	getPdDirectAddToTd : function(n,td_i,title,pic_url,detail_url,price){//添加产品至表格
		var self = this;
		var box = $('#'+self._id+' .'+self._block[n].name+' div div table.productTable tbody > tr > td');
		var changeP =  $(box[td_i]).children('p');
		$(changeP[0]).children('a').children('img').attr('src',pic_url);
		$(changeP[0]).children('a').attr('href',detail_url);
		$(changeP[0]).children('a').attr('target','_blank');
		$(changeP[1]).html(title+'<br /><span style="font-weight:bold; color:#F60;">劲爆促销价：￥'+price+'元</span>');
		self.submitPopClose();
	},
    ...
    getProduct : function(n,source){//在页面写入产品表格
        ...
        if(source=='pop'){//判断来源，来自初始化或Checkbox
            ...
            //for (var i in self._productArray[n]){
			for (var i=0; i<=self._productArray[n].length-1; i++){
				if("undefined" == typeof self._productArray[n][i].title){}//用于识别IE下的产品数组undefined错误。具体为何出现undefined，还不清楚
				else{
					listArray.push(i);
				}
			}
    ...
    getPdList : function(n,pdlist){//格式化Pop框产品列表
        ...
        if (this._productFirst[n] == false){//用于判断是否首次加载，首次加载默认全部选中
            for (var i=0; i<=pdlist.length-1; i++){pdlist[i].checked = "";}
            for (var i=0; i<=this._productListArray[n].length-1; i++){pdlist[this._productListArray[n][i]].checked = "checked='checked'";}
        }else{
            this._productListArray[n] = [];
            for (var i=0; i<=pdlist.length-1; i++){pdlist[i].checked = "checked='checked'";}
            for (var i=0; i<=pdlist.length-1; i++){this._productListArray[n].push(i)}
            this._productFirst[n] = false;
        }
        obj = {}
        //for(var i in pdlist){
		for(var i=0; i<=pdlist.length-1; i++){
            obj.arrayNum = i;
    ...
    popPdPagination : function(n,pageNum){//Pop框产品分页功能
        if(this._productArray[n].length > this._pagination){//判断时候启用分页
            ...
            for (var i=0; i<=this._productArray[n].length-1; i++){$(pdBox[i]).hide();}
	...
	popPdDirectClick : function(p,n,id,td_i){//分页按钮点击事件
        var self = this;
        var menu = $('#'+this._popId+' .pagefen > a');
        $(menu).click(function(){
            var h =$(this).index();
			self.typeFun_product_Direct_pdAdd_ajax(p,n,id,td_i,false);//加载弹窗内的产品列表,参数说明：1:当前页，默认第一页,n:模块号
            $(menu).removeClass('onthis');
            $(this).addClass('onthis');
        })
    },
    ...
    ajaxAddProduct : function(n,id,td_i){//Ajax添加产品， 区块、产品ID、写入的td编号
        var self = this;
                ...
                if(data.enterFlag){
                    if(data.pdlist.length > 0){
						...
						if(td_i == 'old'){
							...
							self.getPdDirectAddToTd(n,td_i,data.pdlist[0].title,data.pdlist[0].pic_url,data.pdlist[0].detail_url,data.pdlist[0].price);
						}
